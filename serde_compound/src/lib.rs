...
fn lets_go() {
    let mut vec = Vec::new();
    let user = User::deserialize(&mut StructDeserializer {
        fields: vec,
        cursor: 0,
    });
    println!("{:?}", user)
...
impl<'de> de::Deserializer<'de> for &mut MapDeserializer<'de> {
    type Error = DeserializeError;
    ...
    forward_to_deserialize_any! {
        ...
        bytes byte_buf option unit unit_struct struct newtype_struct identifier seq tuple
        tuple_struct map enum ignored_any
    }
}
