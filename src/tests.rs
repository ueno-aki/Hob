use crate::protocol::mcpe::{
    ...
    transforms::framer::encode, crypto::cipher::Aes256Ctr64BE,
};
...
fn write_play_status_encrypteed() -> Result<()> {
    let ss:[u8;32] = [97,160,94,73,44,96,222,15,164,99,156,254,55,25,124,119,215,168,192,40,163,56,0,101,223,190,165,130,206,171,178,160];
    let iv: [u8; 16] = [ss.clone()[0..12].to_vec(), vec![0, 0, 0, 2]].concat().try_into().unwrap();
...
fn compute_checksum(){
    ...
    let ss:[u8;32] = [85, 80, 17, 129, 34, 26, 176, 129, 54, 165, 3, 18, 177, 65, 59, 221, 149, 204, 183, 187, 138, 58, 220, 198, 235, 193, 47, 92, 116, 162, 26, 176];
    let counter:u64 = 0;
    let mut digest = hmac_sha256::Hash::new();
    digest.update(counter.to_be_bytes());
    digest.update(plain);
